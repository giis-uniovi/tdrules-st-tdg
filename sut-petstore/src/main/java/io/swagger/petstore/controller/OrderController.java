...
public class OrderController {
    ...
    public ResponseContext addCustomer0(final RequestContext request, final Customer0 customer) {
        if (customer == null) {
            return new ResponseContext()
                    .status(Response.Status.BAD_REQUEST)
                    .entity("No Customer provided. Try again?");
        }

        orderData.addCustomer0(customer);
        return new ResponseContext()
                .contentType(Util.getMediaType(request))
                .entity(customer);
    }

    public ResponseContext placeOrder0(final RequestContext request, final Order0 order) {
        if (order == null) {
            return new ResponseContext()
                    .status(Response.Status.BAD_REQUEST)
        ...
        orderData.addOrder0(order);
        return new ResponseContext()
                .contentType(Util.getMediaType(request))
                .entity(order);
    }
/*
    public ResponseContext placeOrder0(final RequestContext request, final Long id, final Long petId, final Integer quantity, final DateTime shipDate,
    		final String status, final Boolean complete) {
    	final Order0 order = OrderData.createOrder0(id, petId, quantity, new Date(), status, complete);
    	return placeOrder0(request,order);
    }
*/

    //Customer does not have methods to create from the api, include here
    
    public ResponseContext addCustomer(final RequestContext request, final Customer customer) {
        if (customer == null) {
            return new ResponseContext()
                    .status(Response.Status.BAD_REQUEST)
                    .entity("No Customer provided. Try again?");
        ...
        orderData.addCustomer(customer);
        return new ResponseContext()
                .contentType(Util.getMediaType(request))
                .entity(customer);
    }
    ...
    public ResponseContext placeOrder(final RequestContext request, final Long id, final Long petId, final Long customerId, final Integer quantity, final DateTime shipDate,
                                      final String status, final Boolean complete) {
        final Order order = OrderData.createOrder(id, petId, customerId, quantity, new Date(), status, complete);
