 ...
 */
@Slf4j
public class TestMarket extends Base {
	...
	private static boolean testRegionByNameLoaded = false;

	@Test
	public void testRegionByName() {
		if (!testRegionByNameLoaded) {
			load(mvc, "tds RegionDTOReq where name ='Campbeltown'");
			testRegionByNameLoaded = true;
		}
		ApiResponse regions = mvc.get("/testAll");
		assertReadResults(regions);
	}
