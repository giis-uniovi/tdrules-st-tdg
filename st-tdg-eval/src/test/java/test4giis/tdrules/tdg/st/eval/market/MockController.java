...
public class MockController extends ApiWriter {
	...
	public ApiResponse post(String url, String requestBody, boolean usePut) {
		...
		if ("/regiondto".equals(url)) {
			RegionDtoAssembler regionDtoAssembler = new RegionDtoAssembler();
			Region region = regionDtoAssembler.toDomain((RegionDTO)deserialize(requestBody, RegionDTO.class));
			regionService = new RegionServiceImpl(regionDAO);
			regionService.create(region);
			return new ApiResponse(200, "OK", serialize(regionDtoAssembler.toModel(regionService.findByName(region.getName()))));
		}
		/*if ("/pet".equals(url)) {
